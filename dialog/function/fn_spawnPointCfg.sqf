...
switch (_side) do
{
	...
	case west:
	{
		_spawns = _spawns + [
			...
			["cop_spawn_4","Airport HQ","\a3\ui_f\data\map\Markers\NATO\b_air.paa"],
			["cop_spawn_5","Highway HQ","\a3\ui_f\data\map\GroupIcons\badge_rotate_0_gs.paa"]
		];
	};
	...
	case civilian:
	{
		...
		{
			_spawns = _spawns + [
			["civ_adac_spawn","ADAC-HQ","icons\wrench.paa"]
			];
		};

		/*if((["sc"] call life_fnc_permLevel) > 0) then
		{
			_spawns = _spawns + [
			["civ_sc_spawn","SC-HQ","icons\rebel.paa"]
			];
		};*/
		...
		{
			_spawns = _spawns + [
			["reb_spawn","Rebellenlager","icons\rebel.paa"]
		...
		_spawns = _spawns + [
			["civ_spawn_2","Pyrgos (Hauptstadt)","\a3\ui_f\data\map\MapControl\watertower_ca.paa"],	
