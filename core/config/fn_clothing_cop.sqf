...
switch (_filter) do
{
	...
	case 0:
	{		
		_ret set[count _ret,["U_B_CombatUniform_mcam_worn","Polizeiuniform",25]];
	...
	case 1:
	{
		...
		_ret set[count _ret,["H_CrewHelmetHeli_B","Anti-Gashelm",2500]];
		...
		if(__GETC__(life_coplevel) > 4) then
		{
			...
			_ret set[count _ret,["H_Watchcap_camo",nil,150]];
